...
import {
  ArrowLeft, Mail, Calendar, Briefcase, UserCircle, Heart,
  AlertCircle, CheckCircle, Smile, Frown, Meh, RefreshCw,
  Brain, Activity, Clock, Phone, MapPin, FileText
} from 'lucide-react';
  ...
  const fetchClientDetails = async () => {
    ...
    try {
      const data = await apiService({
        url: `${expertBaseUrl}users/${userId}`,
        method: 'GET'
      });
    ...
    <div className="min-h-[80vh] w-full text-white flex flex-col gap-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm">View detailed client information</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-[#1e293b] rounded-xl p-6">
            ...
            <div className="flex flex-col items-center text-center mb-6">
              <div className="w-24 h-24 bg-gradient-to-br from-[#795EFF] to-[#5a3fd4] rounded-full flex items-center justify-center text-3xl font-bold shadow-lg mb-4">
                {client.full_name?.charAt(0) || '?'}
              </div>
              <h2 className="text-xl font-bold">{client.full_name}</h2>
              <p className="text-slate-400 text-sm mt-1">{client.email}</p>
              <span className={`${healthStyle.bg} ${healthStyle.text} text-sm px-3 py-1 rounded-full font-medium mt-3`}>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-[#1e293b] rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-slate-800/50 p-4 rounded-lg">
                ...
                <p className="text-lg font-semibold">{client.age || 'N/A'} years</p>
              </div>
              <div className="bg-slate-800/50 p-4 rounded-lg">
                <p className="text-slate-400 text-xs mb-1">Gender</p>
                <p className="text-lg font-semibold capitalize">{client.gender || 'N/A'}</p>
              </div>
              <div className="bg-slate-800/50 p-4 rounded-lg">
              ...
              <div className="bg-slate-800/50 p-4 rounded-lg">
                ...
                <p className="text-lg font-semibold capitalize">{client.relationship || 'N/A'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#1e293b] rounded-xl p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className={`${emotionalStyle.bg} ${emotionalStyle.border} border rounded-xl p-5`}>
                <div className="flex items-center gap-3 mb-3">
                  <EmotionIcon className={`w-8 h-8 ${emotionalStyle.text}`} />
                  <div>
                    <p className="text-slate-400 text-xs">Current Emotional State</p>
                    <p className={`text-xl font-bold capitalize ${emotionalStyle.text}`}>
              ...
              <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-5">
                <div className="flex items-center gap-3 mb-3">
                  <Activity className={`w-8 h-8 ${healthStyle.bg === 'bg-emerald-500' || healthStyle.bg === 'bg-green-500' ? 'text-green-400' : healthStyle.bg === 'bg-yellow-500' ? 'text-yellow-400' : 'text-red-400'}`} />
                  <div>
                    <p className="text-slate-400 text-xs">Overall Health</p>
                    <p className="text-xl font-bold capitalize">{client.health || 'Unknown'}</p>
                  </div>
                </div>
              </div>
            </div>
