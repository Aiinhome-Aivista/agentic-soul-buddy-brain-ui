...
function SessionDetails() {
  ...
  const handleSubmitInsight = async (e) => {
    ...
    try {
      ...
      console.error('Error submitting expert insight:', err);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="min-h-[80vh] w-full text-white flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1">
        {/* Left Column - Chat Section (2/3) */}
        <div className="lg:col-span-2 flex flex-col gap-4">
          {messages.length === 0 ? (
            <div className="flex-1 flex items-center justify-center bg-[#1e293b] rounded-xl">
            ...
            <div className="flex-1 space-y-4 overflow-y-auto max-h-[65vh] pr-2 bg-[#1e293b] rounded-xl p-6">
        ...
        {/* Right Column - Expert Insight Form (1/3) */}
            ...
            <form onSubmit={handleSubmitInsight} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Enter your professional insight about this session..."
                  rows={8}
                  className="w-full px-4 py-3 bg-slate-800 border border-slate-600 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-[#795EFF] focus:border-transparent transition resize-none"
                  required
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 bg-slate-800 border border-slate-600 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-[#795EFF] focus:border-transparent transition"
                />
                {/* Tags Display */}
                {tags.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {tags.map((tag, index) => (
                      <span
                        key={index}
                        className="inline-flex items-center gap-1 px-3 py-1 bg-[#795EFF]/20 border border-[#795EFF]/50 rounded-full text-sm text-[#795EFF]"
                      >
                        {tag}
                        <button
                          type="button"
              ...
              <button
                type="submit"
                disabled={submitting || !insightText.trim()}
                className="w-full bg-[#795EFF] hover:bg-[#6a4be8] disabled:bg-slate-700 disabled:cursor-not-allowed text-white font-semibold py-3 px-4 rounded-lg flex items-center justify-center gap-2 transition"
              >
                {submitting ? (
                  <>
